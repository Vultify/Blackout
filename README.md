...
The **Admin's Key** spawns on the boss's desk in the manager's office on a blackout raid - single use, opens the arsenal, and the door can't be breached. The manager's office is itself locked, so it's the manager's office key first, then the arsenal. The key itself is WTT-ContentBackport's (the same live item, backported there as of 2.0.1) - Blackout puts it on the desk.
